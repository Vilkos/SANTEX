...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendInvoice(int orderId) {
        ...
        } catch (MessagingException | ParseException | TransformerException | InvalidCodeException | SAXException |
                 DocumentException |
                 ParserConfigurationException | DataIncompleteException | IOException | XMPException e) {
            e.printStackTrace();
        }
    ...
    public void sendRegistrationInfo(int customerId) {
        ...
        if (customer.isPresent()) {
            ...
            try {
                MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");
                helper.setTo(customer.get().getEmail());
